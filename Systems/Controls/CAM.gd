...
export var inverted_y: bool = true
...
# zoom
onready var camera = $position/elevation/zoom/Camera
onready var zoom = $position/elevation/zoom
var zoom_direction = 0
# rotation
onready var elevation = $position/elevation
...
func _zoom(delta: float) -> void:
	if not allow_zoom or not zoom_direction:
		return
	var new_zoom = clamp(
		zoom.translation.z + zoom_direction * zoom_speed * delta,
		min_zoom,
		max_zoom
	)
	var pointing_at = _get_ground_position()
	zoom.translation.z = new_zoom
