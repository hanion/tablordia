...
export var materyal0: SpatialMaterial
export var materyal1: SpatialMaterial
...
func change_mat(index: int) -> void:
	base_index = index
	
	if index == 0:
		materyal = materyal0.duplicate(true)
	elif index == 1:
		materyal = materyal1.duplicate(true)


func change_item(index: int) -> void:
	if base_index == 0:
		materyal.set_uv1_offset( items[index] )
	elif base_index == 1:
		materyal.set_uv1_offset( items1[index] + Vector3(0,-0.003,0) )
		
	$mesh.set_material_override(materyal)


func notify_dispenser() -> void:
	change_mat(item_base)
